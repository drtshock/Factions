...
/*		// this code was really slow on large servers, getting full info for every faction and then only showing 9 of them; rewritten below
        lines.add(plugin.txt.parse("<i>Factionless<i> %d online", Factions.i.getNone().getFPlayersWhereOnline(true).size()));
		for (Faction faction : factionList)
		{
			lines.add(plugin.txt.parse("%s<i> %d/%d online, %d/%d/%d",
				faction.getTag(fme),
		...
		sendMessage(plugin.txt.getPage(lines, this.argAsInt(0, 1), "Faction List"));
 */
